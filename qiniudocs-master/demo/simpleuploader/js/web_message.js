if (typeof console == "undefined") {    this.console = { log: function (msg) {  } };}
    ...
    function onmessage(e)
    {
        ...
        switch(data['type']){
            ...
            case 'login':
                ...
                break;
            case 'say_uid':
                say_uid(data['image'], data['mestype'], data['header_img_url'],data['group_name'], data['session_no'], data['insert_id'], data['from_uid_id'], data['to_uid_id'], data['from_client_id'], data['from_client_name'], data['content'], data['time']);
                break;
            //审核消息
            case 'audit':
    ...
    */
    ...
    //  提交会话
    function onSubmit(to_uid, uid, groupId, accept_name, message_type, mes_types,from_session_no) {
    ...
    function flush_onlineman_list(){
        ...
        for(var p in client_list){
            online_ren ++;
            onlineman_ul.append('<li id="'+p+'" class="chat_people" group-name="'+client_list[p].client_name+'"><span class="header-img"><img src="'+client_list[p].header_img_url+'" alt="'+client_list[p].client_name+'"></span>'+client_list[p].client_name+'</li>');
        }
        $('.online_ren').html(online_ren);
    }
    //发言2
    function say_uid(image, mestype, header_img_url,group_name, from_session_no, insert_id, from_uid_id, to_uid_id, from_client_id, from_client_name, content, time){
      if (image == 'image') {
        content1 = '【图片】';
      } else {
      ...
      if (mestype == "message") {
        showMsgNotification(from_client_name, content1, document_url);
        $(".mes_con").append('<div class="mes_box"><span class="mex_con">'+from_client_name+':'+content1+'</span><div style="height:30px"><span mestype="'+mestype+'" mesid = "'+insert_id+'" group-name="'+from_client_name+'" group-all="'+to_uid_id+'" id="'+from_uid_id+'" session_no="'+from_session_no+'" class="mes_chakan_close chat_people">查看</span></div><span session_no="'+from_session_no+'" mesid= "'+insert_id+'" mestype="'+mestype+'" class="mes_close">X</span></div>');
      } else {
          showMsgNotification(group_name, content1, document_url);
          $(".mes_con").append('<div class="mes_box"><span class="mex_con">'+group_name+':'+content1+'</span><div style="height:30px"><span mestype="'+mestype+'" mesid = "'+insert_id+'" group-name="'+group_name+'" group-all="'+to_uid_id+'" id="'+from_uid_id+'" class="mes_chakan_close session_no" session_no="'+from_session_no+'">查看</span></div><span session_no="'+from_session_no+'" mesid= "'+insert_id+'" mestype="'+mestype+'" class="mes_close">X</span></div>');
      };
      if (session_no == from_session_no) {
        $(".he_ov").append('<li class="Chat_le"><div class="user"><span class="head le"><span class="header-img"><img src="'+header_img_url+'" alt=""></span></span> <span class="name le">'+from_client_name+'<span style="padding: 0 0 0 20px">'+time+'</span></span><div class="mes_content le"><span class="jian le"></span> <span class="content-font le">'+content.replace(/%5C/g,"\\").replace(/%6b/g,"<br/>")+'</span></div></div></li>');
      };
      $(".he-ov-box").scrollTop($(".he-ov-box")[0].scrollHeight);
       //消息加一
       mesnum++;
    ...
    function mes_chat(data){
      ...
      for (var i in data) {
        ...
        if (data[i].sender_id == uid) {
          ...
          $(".he_ov").prepend('<li class="Chat_le"><div class="user"><span class="head le"><span class="header-img"><img src="'+data[i].card_image+'" alt=""></span></span> <span class="name le">'+data[i].sender_name+'<span style="padding: 0 0 0 20px">'+mes_time+'</span></span><div class="mes_content le"><span class="jian le"></span> <span class="content-font le">'+data[i].message_content.replace(/%5C/g,"\\").replace(/%6b/g,"<br/>")+'</span></div></div></li>');
        }
      }
    }
    //加载消息
