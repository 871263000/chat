...
jQuery(document).ready(function ($) {
    $("#spig").mousedown(function (e) {
        if(e.which==3){
          showMessage("<span class='man_hid'>隐藏</span>",10000);
          $('.man_hid').click(function (){
            showMessage("拜拜了 ",8000);
            $("#spig").animate({
              left: $(window).width(),
              opacity: 0
            }, 2000)
...
//无聊讲点什么
jQuery(document).ready(function ($) {
  // console.log($(window).height()/2*1.75);
$(".spig").css('left', $(window).height()/2*1.75);

    window.setInterval(function () {
        msgs = ["陪我聊天吧！", "<a href=\"http://messense.me/feed/\" target=\"_blank\" rel=\"external\" tip=\"Feed\"><img border=\"0\" title=\"订阅乱了感觉\" alt=\"Feed\" src=\"/wp-content/themes/chromeStyle/images/rss.png\"></a>", "好无聊哦，你都不陪我玩！", "…@……!………", "^%#&*!@*(&#)(!)(", "我可爱吧！嘻嘻!~^_^!~~","谁淫荡呀?~谁淫荡?，你淫荡呀!~~你淫荡！~~","从前有座山，山上有座庙，庙里有个老和尚给小和尚讲故事，讲：“从前有座……”"];
        var i = Math.floor(Math.random() * msgs.length);
...
jQuery(document).ready(function ($) {
    window.setInterval(function () {
        ...
        var i2 = Math.random();
            $(".spig").animate({
              // left: $(window).width(),
              // top: winWidth+$(window).scrollTop()
            left: $(window).width()*i1,
            top:  (winWidth+$(window).scrollTop())*i2
        },
			{
			    duration: 2000,
