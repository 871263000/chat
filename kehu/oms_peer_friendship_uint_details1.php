<?php
    ...
    //var_dump($row['input_person']);
       //上一篇下一篇
    $sql_pref="SELECT * FROM  {$tabName} where oms_id=$oms_id and `state`=0 and {$idName}<'{$id}' and $where  order by id desc limit 0,1";

    $sql_next="SELECT * FROM  {$tabName} where oms_id=$oms_id and `state`=0 and {$idName}>'{$id}' and $where  order by id asc limit 0,1";
    ...
    if ($res) {
      //上一篇
      $pref="上一篇<a href='oms_peer_friendship_uint_details1.php?id={$row1['id']}&start={$start}&stop={$stop}&Vehicle_id={$peer_name}'>{$row1['peer_name']}</a>";
    }else{
      $pref="上一篇 没有了";
    }
    ...
    if ($res) {
      //下一篇
      $next="下一篇<a href='oms_peer_friendship_uint_details1.php?id={$row2['id']}&start={$start}&stop={$stop}&Vehicle_id={$peer_name}'>{$row2['peer_name']}</a>";
    }else{
      $next="下一篇 没有了";
    }
...
<!--  end -->
   <h2 id="tables-bordered" class="text-center">同行及友情单位列表详情<a class="fanhui" href="./oms_peer_friendship_uint_list1.php">列表页</a></h2>
   <div class="container">
  <table class="table table-bordered table-striped  table-hover"> 
  <form class="form-horizontal" action='oms_peer_friendship_uint_action1.php?id=<?php echo $id?>' method='post'  enctype="multipart/form-data">
        <tr>
        ...
        <tr>
           <td colspan="10" style="text-align:center"><p style="float:left" ><?php echo $next;?></p>
           <a class="btn btn-info" href="./oms_peer_friendship_uint_list1.php?id=<?php echo $id;?>" >返回</a> 
           <a class="btn btn-primary" href="./oms_peer_friendship_uint_edit1.php?id=<?php echo $id;?>" >我要修改</a> 
          <p style="float:right" ><?php echo $pref;?></p>
           </td>
        </tr>
    </form>
   </table> 
