...
class Page
{
	...
	private function indexPage()
	{
		...
		return '<a href="'.$this->url.'1'.$this->anchor.'">首页</a>';
	}
	...
	private function endPage()
	{
		...
		return '<a href="'.$this->url.$this->pageAmount.$this->anchor.'">尾页</a>';
	}
	...
	{
		if($this->currentPage == 1) return;
		return '<a href="'.$this->url.( $this->currentPage - 1 ).$this->anchor.'">上一页</a>';
	}
	...
	{
		if($this->currentPage == $this->pageAmount) return;
		return '<a href="'.$this->url.( $this->currentPage + 1 ).$this->anchor.'">下一页</a>';
	}
	...
	private function pageNumber()
	{
		...
		if($this->pageAmount > ($this->offset * 2 + 1))
		{
			...
			return $left.'<a href="'.$this->url.$this->currentPage.'" '.$this->classHere.$this->anchor.'>'.$this->currentPage.'</a>'.$right;
		}
		else
		{
			$allLink='';
			...
			{
				 $allLink.='<a href="'.$this->url.$j.$this->anchor.'" '.($j == $this->currentPage?$this->classHere:'').'>'.$j.'</a>';
			}
			return $allLink;
		}
	}
