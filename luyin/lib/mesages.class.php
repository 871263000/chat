...
class messageList
{
	...
	*/
	...
	public function mesAlertList(){
		...
		//单聊消息
		$sql = 'SELECT a.`mes_num`, a.`session_no`,a.`pid`, b.`sender_id`, b.`sender_name`, b.`accept_id`, b.`accept_name`, b.`message_type`, b.`mesages_types` , b.`message_content`, b.`groupId` FROM `oms_chat_message_ist` a LEFT JOIN `oms_string_message` b ON a.`mes_id` = b.`id` WHERE a.`pid`="'.$this->uid.'"';
