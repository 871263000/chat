...
<style type="text/css">
  *{font-family: 'Microsoft YaHei', SimSun, sans-serif !important;}
  ...
  <script type="text/javascript">
    ...
    var session_no = "<?php echo $session_no?>";
    var document_url = "<?php echo DOCUMENT_URL?>";
  </script>
</head>
<body>
    ...
    <div class="container">
    <div class="mes_title">
    <span id='s_man' title='选择人聊天' class='mes_ren' ></span>
    <span id='mes_ren' title='选择群聊' class='mes_rens'></span>
    <a href="new_groupChat.php" target="_blank"><span id='groupMan' title='新建群聊' class='groupMan'></span></a>
   ...
   <!-- 群列表 --> 
    <div class="group-list">
      <div class="man_tittle">
        <span>群列表</span>
        <span class ="group_List" style='cursor: pointer; margin-right: 5px;background: #000;color: #fff; position:absolute;width:20px;height:20px;border-radius: 50%;right: 0; top: 7px; '>X</span>
      </div>
      <ul class="group_pep list-group">
      <?php 
        if (!empty($arrGroup)) {
          foreach ($arrGroup as $key => $value) {
            echo "<li class='session_no' group-all='".$value['all_staffid']."' session_no='".$value['pid']."'>".$value['group_name']."</li>";
          }
        }
       ?>
      </ul>
...
//群列表显示
$('#mes_ren').click(function (){
  $('.group-list').show();
})
//群列表关闭
$('.group_List').click(function (){
  $(this).parents('.group-list').hide();
})
//群聊拖动
var _move = false;
...
$('#submit').click(function (){
  ...
  onSubmit(to_uid, uid, mes_type, session_no);
});
